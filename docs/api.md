...
- 传入：选中仓库id列表、冶炼厂id列表、品类id列表、price_type（取价口径）、**吨数**（可选，默认 `1`）。**总运费** = **每吨运费**×**吨数**（即 **运费×吨数**）；运费表字段 `price_per_ton` 为 **元/吨**。展示用 **`报价`** 为折合后的 **元/吨**（不含税）；**`报价金额`** = **报价×吨数**；**`利润`** = **`报价金额` − `总运费`**（即 **报价×吨数 − 运费×吨数**）。**最终比价**以 **`利润`** 为准；明细按「最优价排序口径」下利润排序；**`冶炼厂利润排行`** 为各冶炼厂明细 **`利润`** 之和（默认排行键与所选最优价口径一致，见返回字段）。**`最优价各口径利润`** = 该口径元/吨单价×吨数 − **总运费**（与主行共用同一 **`总运费`**）。
- 输出：
  - `data`：明细列表（含 `运费`（**元/吨**）、`总运费`、`报价金额`、`运费计价方式`（固定 `per_ton`）、`利润`、`最优价各口径利润` 等；若 `price_type` 为含 1%/3%/13% 增值税，先将含税价折合为不含税再写入 `报价`；无报价时 `报价` / `报价金额` 为 `null`，`报价来源` 为 `unavailable`，此时 **`利润`** = **−`总运费`**
  - **`冶炼厂利润排行`**：按冶炼厂汇总明细中的 `利润` 之和，**从高到低**排序，每项含 `冶炼厂id`、`冶炼厂`、`利润`
  - **前端提示**：列表若展示「到库利润 / 总成本」类指标，请直接绑定接口返回的 **`利润`**（或采购建议里的 **`比价利润(元)`**），**不要**在前端用 **`报价(元/吨) + 运费(元/吨)`** 自行加算；本系统运费为 **元/吨**，列头应与接口一致。
...
```json
{
  "code": 200,
  "data": [
    { "仓库id": 101, "冶炼厂id": 201, "品类id": 301, "仓库": "北京仓", "冶炼厂": "华北冶炼厂", "品类": "铜", "price_type": "3%增值税", "吨数": 10.5, "运费": 200, "总运费": 2100, "报价": 9453.4, "报价金额": 99260.7, "报价来源": "direct", "利润": 97160.7 },
    { "仓库id": 101, "冶炼厂id": 201, "品类id": 302, "仓库": "北京仓", "冶炼厂": "华北冶炼厂", "品类": "铝", "price_type": "3%增值税", "吨数": 10.5, "运费": 200, "总运费": 2100, "报价": 8106.8, "报价金额": 85121.4, "报价来源": "calc_from_base", "利润": 83021.4 },
    { "仓库id": 102, "冶炼厂id": 201, "品类id": 301, "仓库": "上海仓", "冶炼厂": "华北冶炼厂", "品类": "铜", "price_type": "3%增值税", "吨数": 10.5, "运费": 300, "总运费": 3150, "报价": null, "报价金额": null, "报价来源": "unavailable", "利润": -3150.0 }
  ],
  "冶炼厂利润排行": [
...
- 逻辑说明：
  ...
  2. **整组替换**：当 `品类id > 0` 时，该分组下原先启用、但 **未出现在本次 `品类名称` 列表中的别名** 会软删除（`is_active=0`）；列表内名称为最终保留的别名集合
  3. 该 `category_id` 下相关记录的 `is_main` 先全部置 0，再按列表顺序写回主名称
  4. 名称在库中已存在（任意分组、含已停用行）则更新其 `category_id`、`is_main` 并置为启用；不存在则插入新行
  5. 列表会去重（保留首次出现顺序），首尾空白会去掉；单项不能为空串，长度 ≤50
- 模拟请求JSON：
```json
[
