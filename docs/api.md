...
- 传入：选中仓库id列表、冶炼厂id列表、品类id列表、price_type（取价口径）、**吨数**（可选，默认 `1`）、**每车吨数**（可选，默认 `35`）。**总运费**恒为：**车数**×**每车运费**；车数 = `max(1, ⌈吨数 / 每车吨数⌉)`。运费表字段 `price_per_ton` 在比价中按 **元/车（每车运费）** 使用。报价侧单价仍为 **元/吨**（不含税展示用）；**`利润`** = 不含税报价×吨数 − **总运费**；**`最优价各口径利润`** = 该口径元/吨单价×吨数 − **总运费**（与主利润共用同一总运费）。
- 输出：
  - `data`：明细列表（含 `车数`、`每车吨数`、`运费`（**元/车**）、`总运费`、`运费计价方式`（固定 `per_truck`）、`最优价各口径利润` 等；若 `price_type` 为含 1%/3%/13% 增值税，先将含税价折合为不含税再写入 `报价`；无报价时 `报价` 为 `null`，`报价来源` 为 `unavailable`）
