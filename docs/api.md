...
- 传入：选中仓库id列表、冶炼厂id列表、品类id列表、price_type（取价口径）、**吨数**（可选，默认 `1`）、**报价日期**（可选 `YYYY-MM-DD`；传入则只取该日的 `quote_details`，不传则以比价基准日（默认 `Asia/Shanghai` 当天，环境变量 `QUOTE_COMPARISON_TZ`）为参照，每个冶炼厂+品种在 `quote_details` 中取 **与该日日历距离最近** 的 `quote_date` 一条；若多条距离相同，则取 **`created_at` 最新** 的一条）。计价关系：**总价** = **单价**×**吨数**（与 **`报价金额`** 一致，**`单价`/`报价`** 为折合后元/吨不含税）；**运费** = **运费单价**×**吨数**（与 **`总运费`** 一致，**`运费单价`** 来自 `freight_rates.price_per_ton`，元/吨）；**利润** = **总价 − 运费**（与 **`报价金额` − `总运费`** 一致）。**最终比价**以 **`利润`** 为准；明细按「最优价排序口径」下利润排序；**`冶炼厂利润排行`** 为各冶炼厂明细 **`利润`** 之和。**`最优价各口径利润`** = 该口径元/吨单价×吨数 − **总运费**（与主行共用同一全程运费金额）。
- 输出：
